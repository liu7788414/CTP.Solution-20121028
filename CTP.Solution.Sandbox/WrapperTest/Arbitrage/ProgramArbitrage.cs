...
[assembly: log4net.Config.XmlConfigurator(ConfigFile = "WrapperTestArbitrage.exe.config", Watch = true)]
...
namespace WrapperTest
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                var processes = Process.GetProcessesByName("WrapperTestArbitrage");
    ...
    ((TraderAdapter)Utils.Trader).TradingDay);
                ...
                Utils.WriteLine("订阅套利合约行情", true);
                var array = new List<string>();
                array.Add(Utils.A1);
                array.Add(Utils.A2);

                ((QuoteAdapter)Utils.QuoteMain).SubscribeMarketData(array.ToArray());
                ((QuoteAdapter)Utils.QuoteMain).SubscribedQuotes.AddRange(array);
