...
namespace WrapperTest
{
    public class TraderAdapter : CTPTraderAdapter, ITraderAdapter
    {
            ...
            try
            {
                ...
                foreach (var order in UnFinishedOrderFields.Values)
                {
                    ...
                    if ((order.CombOffsetFlag_0 == EnumOffsetFlagType.Open) && (order.OrderStatus == EnumOrderStatusType.NoTradeQueueing || order.OrderStatus == EnumOrderStatusType.PartTradedQueueing))
                    {
                        ...
                        if (DateTime.Now + new TimeSpan(2, 0, Utils.ExchangeTimeOffset) - updateTime > new TimeSpan(0, 0, 10))
                        {
        ...
        public void OpenPositionByInstrument(string instrumentId, string reason, EnumPosiDirectionType longOrShort, double openTrendStartPoint, bool bConsiderOppositePosition, bool bForceOpen, double openAngle, double price)
        {
            try
            {
                Utils.WriteLine("OpenPositionByInstrument", true);
                ...
                if (Utils.IsInInstrumentTradingTime(instrumentId) && !PositionFields.ContainsKey(keyToday) && !PositionFields.ContainsKey(keyHistory)) //没有空仓，卖出开仓
                {
        ...
        public bool ContainsTodayPositionByInstrument(string instrumentId, EnumPosiDirectionType longOrShort)
        {
            var keyToday = Utils.GetPositionKey(instrumentId, longOrShort, EnumPositionDateType.Today);
            return PositionFields.ContainsKey(keyToday);
        }

        public bool ContainsYesterdayPositionByInstrument(string instrumentId, EnumPosiDirectionType longOrShort)
        {
            var keyHistory = Utils.GetPositionKey(instrumentId, longOrShort, EnumPositionDateType.History);
            return PositionFields.ContainsKey(keyHistory);
        }
        ...
        public void ClosePositionByInstrument(string instrumentId, string reason, EnumPosiDirectionType longOrShort, double offset, double price = 0)
        {
            try
            {
                Utils.WriteLine("ClosePositionByInstrument", true);

                var keyToday = Utils.GetPositionKey(instrumentId, longOrShort, EnumPositionDateType.Today);
                var keyHistory = Utils.GetPositionKey(instrumentId, longOrShort, EnumPositionDateType.History);

                if ((Utils.IsInInstrumentTradingTime(instrumentId)) &&
                    (PositionFields.ContainsKey(keyToday) || PositionFields.ContainsKey(keyHistory)))
                {
                    Utils.WriteLine("PositionFields", true);

                    //平今仓
                    if (PositionFields.ContainsKey(keyToday))
                    {
                        Utils.WriteLine("closetoday", true);
                    ...
                    {
                        Utils.WriteLine("closeyesterday", true);
        ...
        {
            ClosePositionByInstrument(instrumentId, reason, EnumPosiDirectionType.Long, -10, price);
        }
        ...
        {
            ClosePositionByInstrument(instrumentId, reason, EnumPosiDirectionType.Short, -10, price);
        }
                        ...
                        case EnumOffsetFlagType.Open:
                            {
                                ...
                                if (PositionFields.ContainsKey(keyToday))
                                {
                                    ...
                                    if (Utils.InstrumentToInstrumentInfo.ContainsKey(pTrade.InstrumentID))
                                    {
                                        var info = Utils.InstrumentToInstrumentInfo[pTrade.InstrumentID];
                                        positionToday.OpenCost = (positionToday.OpenCost + pTrade.Price * pTrade.Volume * info.VolumeMultiple);
                                    }
            ...
            EnumVolumeConditionType volumeCondition = EnumVolumeConditionType.AV, double priceIn = 0)
        {
            try
            {
                Utils.WriteLine("OrderInsertOffsetPrice", true);
